...
_ASYNC_MILVUS_CLIENT_v1 = WeakKeyDictionary()
_MILVUS_LOCK_v1 = asyncio.Lock()

async def milvus_client_v1():
    loop = asyncio.get_running_loop()

    async with _MILVUS_LOCK_v1:
        _client = _ASYNC_MILVUS_CLIENT_v1.get(loop)
        if _client is None:
            _client = AsyncMilvusClient(
                uri=CLIENT_URI,
                token=CLIENT_TOKEN
            )
            _ASYNC_MILVUS_CLIENT_v1[loop] = _client
        return _client
...
class MilvusVectorStoreConnection:
    ...
    # async def check_properties(self):
        # client = await milvus_client_v1()
        # props = await client.describe_collection(collection_name=self.collection_name)
        # # print raw for debugging (remove in production)
        # print("Raw describe_collection:", props)
        # properties = props.get("properties", {})
        # return properties.get("collection.ttl.seconds")

    async def check_properties(self):
        client = await milvus_client_v1()
        props = await client.describe_collection(collection_name=self.collection_name)
        properties = props.get("properties", {})
        val = properties.get("collection.ttl.seconds")
